...
robot1.place(0, 0, 'NORTH')
robot2.place(0, 1, 'NORTH')

# PUSH 操作
robot1.execute("PUSH 1")  # A: (0,0)→(0,1), B: (0,1)→(0,2)

# 现在两个机器人都有独立的历史
# robot1.history = [..., (0,0,NORTH,0,[robot2.id])]
# robot2.history = [..., (0,1,NORTH,0,[])]

# 问题：谁来 UNDO？结果应该是什么？
robot1.execute("UNDO")  # A 回到 (0,0)？B 也要回到 (0,1) 吗？
robot2.execute("UNDO")  # B 独立 UNDO 回到 (0,1)？还是拒绝？
```
...
```python
robot1.execute("UNDO")  # A: (0,1)→(0,0), move_count: 1→0
robot2.execute("UNDO")  # B: (0,2)→(0,1), move_count: 1→0
```

**特点**：每个机器人独立 UNDO，互不影响

**问题**：
- ❌ **语义混乱**：PUSH 是"原子操作"，但 UNDO 却分开了
- ❌ **不一致**：UNDO A 后变成 A(0,0) B(0,2)，这不是 PUSH 前的状态
- ❌ **用户困惑**：期望"UNDO PUSH"会同时恢复两个机器人

**代码**：简单，就是当前的做法

---

#### **方案 B：推动者负责 UNDO（一主一从）** ← **v2 首选**

```python
robot1.execute("UNDO")  # A 协调 UNDO
# 1. B: (0,2)→(0,1), move_count: 1→0
# 2. A: (0,1)→(0,0), move_count: 1→0

robot2.execute("UNDO")  # B 拒绝
# {"success": False, "message": "This robot was pushed by Robot A; undo via Robot A"}
```

**特点**：推动者负责整个 PUSH 的 UNDO

**优点**：
- ✓ **语义清晰**：PUSH 是原子的，UNDO 也是原子的
- ✓ **一致性**：UNDO 后两个机器人都回到 PUSH 前状态
- ✓ **易理解**："你推了别人，你负责 UNDO"

**问题**：
- ❌ **实现复杂**：需要反向索引"我是被谁推的"
- ❌ **灵活性差**：B 不能独立操作后再 UNDO
- ❌ **后续状态**：如果 B 被推后又 MOVE 了，UNDO PUSH 时 B 的 MOVE 怎么办？

**代码需求**：
```python
# 需要添加反向索引
robot.pushed_by = (pusher_id, push_operation_index)
...
def undo(self):
    ...
    if affected_robots:
        for robot_id in affected_robots:
            affected_robot = self.table.get_robot_by_id(robot_id)
            affected_robot.undo_as_pushed()  # 协调 UNDO
```

---

#### **方案 C：协调式 UNDO（两个都能发起）**

```python
robot1.execute("UNDO")  # A 发起
# → 同时恢复 A 和 B

robot2.execute("UNDO")  # B 发起（如果之前没被 UNDO）
# → 反向查找推动者 A，协调 UNDO
```

**特点**：任何一方都可以发起 UNDO，系统自动找到配对
...
# 不作为 A 的一条历史
# 而是分别记录：
#   - A.history.append((0,0,...))  # A 移动了
#   - B.history.append((0,1,...))  # B 也移动了
# 都标记为由 PUSH 引发，但分开记录
```
...
1. **原子性 vs 独立性的矛盾**
   - PUSH 修改了两个独立对象的状态
   - 如果 UNDO 时其中一个位置被占据，无法同时恢复

   ```python
   robot1.execute("PUSH 1")  # A: (0,0)→(0,1), B: (0,1)→(0,2)
   robot3.place(0, 0, 'SOUTH')  # 占据了 A 原来的位置
   robot1.execute("UNDO")  # 无法回到 (0,0)，因为已被占据
   ```

2. **后续状态污染**
   - PUSH 后，B 可能又改变了状态
   - UNDO 时是否应该回滚 B 的所有后续操作？

   ```python
   robot1.push(2)  # A 推 B
   robot2.move()   # B 又移动了
   robot1.undo()   # 应该把 B 的 MOVE 也回滚吗？
   ```

3. **多步链式的复杂性**
   ```python
   robot1.push(2)  # A 推 B
   robot2.push(3)  # B 推 C（B 本身被推，但又推了别人）
   
   robot1.undo()   # A 想回滚自己的 PUSH
   # 需要协调 B 和 C，但 C 也在链中...
   ```

4. **历史条目的同步问题**
   - A 的 PUSH 在 A 的历史中是 1 条
   - B 的被推动在 B 的历史中也是 1 条
   - 但它们需要同步 UNDO
   - 如果中间有其他机器人的干扰，同步会失败

### 最终选择

| 方案 | 实现成本 | 语义清晰 | 扩展性 | 可维护性 | 选择 |
|------|---------|---------|--------|---------|------|
| A：独立 UNDO | 低 | 差 | 低 | 高 | ✓ 当前 MVP |
| B：推动者负责 | 中 | 好 | 中 | 中 | ✓ v2 推荐 |
| C：协调式 | 高 | 好 | 高 | 低 | △ v3 可考虑 |
| D：分离操作 | 中 | 差 | 低 | 差 | ✗ 不选 |

**现状**：
- ✓ 当前：方案 A（独立 UNDO）
- 🔮 未来：可升级到方案 B（推动者负责）
- ⚠️ 折衷：接受"UNDO PUSH"的不完美，换来实现的简洁
...
class Robot:
    def __init__(self, table, name="anonymous"):
        # ... 现有代码 ...
        self.pushed_by = None  # (pusher_id, operation_index)
```
...
def push(self, steps=1):
    # ... 验证逻辑 ...
    if next_validation.get("success"):
        blocked_robot = self.table.get_robot_by_position(new_x, new_y)
        
        # 新增：标记被推机器人
        blocked_robot.pushed_by = (self.id, len(self.history))
        
        blocked_robot.append_history()
        blocked_robot.move_to(next_x, next_y)
        
        self.append_history(affected_robots=[blocked_robot.id])
...
def undo(self):
    ...
    if affected_robots:
        ...
        for robot_id in affected_robots:
            ...
            if affected_robot:
                # 验证这个机器人确实是被我推的
                if affected_robot.pushed_by == (self.id, len(self.history)):
                    # 协调 UNDO 被推机器人
                    if not affected_robot._undo_after_push():
                        all_success = False
                    affected_robot.pushed_by = None
        
        if not all_success:
            # 协调失败，回滚自己的 UNDO
            self.history.append(prev_state)
            return {"success": False, 
                    "message": "Cannot undo PUSH: affected robot blocked"}
...
**4. 被推机器人的协调 UNDO**
```python
def _undo_after_push(self):
    """Internal: undo caused by being pushed (coordinated with pusher)."""
    if not self.history:
        return False
...
def undo(self):
    ...
    # 如果我正被某个机器人推动，拒绝独立 UNDO
    if self.pushed_by:
        pusher_id, _ = self.pushed_by
        pusher = self.table.get_robot_by_id(pusher_id)
        return {
            "success": False,
            "message": f"This robot was pushed by {pusher.name}; "
                      f"undo via {pusher.name}"
        }
...
1. **两个独立对象的联合操作**
   - A 和 B 是独立的 Robot 对象
   - PUSH 是 A 发起的，但修改了 B 的状态
   - 打破了"对象只负责自己"的原则

2. **原子性 vs 独立性的永恒矛盾**
   - 用户期望 PUSH 是原子的（两个一起动）
   - 代码设计中 UNDO 是独立的（各自调用 undo()）
   - 这两个需求无法同时完美满足

3. **后续状态的" 污染"**
   - PUSH 后 B 可能又改变
   - UNDO PUSH 时要不要回滚 B 的后续改变？
   - 没有清晰的答案
...
| **UNDO PUSH** | **不支持（当前 MVP）** | **原子性 vs 独立性矛盾；方案 B 留给 v2** |
...
### 坑 12：UNDO PUSH 的"两个对象、一个操作"困境（设计纠结）

原问题：PUSH 修改了两个机器人的状态，但 UNDO 是独立的。如何保证原子性？

**困境的根源**：
- PUSH 是一个逻辑上的原子操作（"推动某物"）
- 但它涉及两个独立的 Robot 对象
- UNDO 的设计中，每个机器人都有独立的历史栈
- 这两个设计目标无法同时完美满足

**当前解决方案**：暂不实现 PUSH UNDO

```python
# 当前行为
robot1.push(2)  # A: (0,0)→(0,1), B: (0,1)→(0,2)
robot1.undo()   # A: (0,1)→(0,0), B 不变
robot2.undo()   # B: (0,2)→(0,1)
# 结果：A(0,0) B(0,1)，巧合等于 PUSH 前
# 但如果 B 之后又移动过，就会出现不一致
```

**为什么这么难？**
1. 两个对象的独立性 vs 操作的原子性矛盾
2. 后续状态污染（B PUSH 后又移动了）
3. 多个碰撞检测点（UNDO 时两个位置都要有效）
4. 链式操作爆炸（A→B→C 的链式推导）

**未来方案**：见《UNDO PUSH 的设计困境》完整章节

**教训**：有些设计困难需要接受"不完美"的 MVP，而不是试图一次性解决。关键是清晰地界定当前的限制，为未来的改进预留设计空间。
...
1. **实现 PUSH UNDO（方案 B - 推动者负责）** ← 优先级最高
   - 添加反向索引：`robot.pushed_by = (pusher_id, operation_index)`
   - PUSH 时设置反向索引追踪关系
   - UNDO 时协调恢复被推机器人
   - 被推机器人拒绝独立 UNDO（告知推动者）
   - 新增测试：test_robot_pushing_and_undo（当前被注释）
   - 核心设计文档：见《UNDO PUSH 的设计困境》章节
...
3. **Robot 移除接口**
   - 实现 `table.remove_robot(robot_id)` 清理网格
   - 从 robot_registry、robots 网格中移除
   
4. **动态障碍物**
   - 运行时添加/删除障碍物（当前是初始化时固定）
   
5. **查询接口**
   - 提供 `get_all_robots()`、`get_robots_at_position(x, y)` 等接口
...
**Q7：UNDO PUSH 为什么这么复杂？**
- A："这是我最有趣的设计矛盾。PUSH 涉及两个独立的 Robot 对象，但需要原子性更新。如果被推机器人之后又移动了，UNDO 是否应该回滚它？如果恢复位置被占据怎么办？

我对比了四个方案：(A) 独立 UNDO 简单但语义混乱，(B) 推动者负责 UNDO 清晰但需要反向索引，(C) 协调式 UNDO 灵活但极其复杂，(D) 分离操作会丢失语义。

我的选择是暂不实现 PUSH UNDO（当前仅支持独立 UNDO），为 v2 预留方案 B 的设计空间。这体现了'务实的 MVP 精神'——清晰的边界、不过度设计、等到真正有需求再升级。"
